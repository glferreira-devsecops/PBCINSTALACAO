...
<nav id="mobile-menu-panel" [class.translate-x-0]="vm.isMenuOpen" [class.translate-x-full]="!vm.isMenuOpen"
  ...
  <div class="flex items-center justify-between p-4 border-b border-slate-200">
    ...
    <button #closeBtn (click)="toggleMenu()" class="p-2 -mr-2 rounded-full text-slate-500 hover:bg-slate-100 hover:text-dark transition-colors" aria-label="Close menu">
      ...
      @for (s of vm.sections; track s.id) {
        <li>
          <button
            (click)="scrollTo(s.id)"
            [attr.aria-current]="vm.activeSection === s.id ? 'page' : null"
            [class]="'flex items-center justify-between w-full text-left text-lg font-medium p-4 rounded-lg hover:bg-slate-100 transition-colors duration-200 ' + (vm.activeSection === s.id ? 'bg-primary/10 text-primary font-bold' : 'text-slate-700')">
            <span>{{ s.label }}</span>
...
<main role="main">
  ...
  <section id="hero" class="hero relative min-h-screen flex items-center text-white overflow-hidden">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center z-10">
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up animate-delay-400">
        <a [href]="vm.whatsapp.normal" target="_blank" rel="noopener noreferrer" class="bg-accent text-dark font-bold text-lg py-4 px-8 rounded-lg hover:scale-105 transition-transform duration-200 shadow-2xl">
  ...
  <!-- TRUST BAR -->
  <section id="trust" aria-labelledby="trust-heading" class="bg-light border-b border-slate-200">
  ...
  <section id="services" aria-labelledby="services-heading" class="py-24 lg:py-32 bg-white">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-16" appAnimateOnScroll>
        <h2 id="services-heading" class="text-4xl lg:text-5xl font-black text-primary">Nossos Serviços Especializados</h2>
  ...
  <!-- PROCESS -->
  <section id="process" aria-labelledby="process-heading" class="py-24 lg:py-32 bg-light">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-20" appAnimateOnScroll>
        <h2 id="process-heading" class="text-4xl lg:text-5xl font-black text-primary">Nosso Processo: Simples e Eficiente</h2>
      ...
      <div class="relative max-w-5xl mx-auto">
        <div class="hidden md:block absolute top-12 left-0 w-full h-0.5 bg-primary/20" aria-hidden="true"></div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
          @for (step of vm.processSteps; track step.title; let i = $index) {
            <div class="flex flex-col items-center text-center relative" 
                 appAnimateOnScroll
                 [style.transition-delay]="i * 100 + 'ms'">
              <div class="flex items-center justify-center w-24 h-24 bg-white rounded-full border-4 border-primary shadow-lg z-10">
                <span class="text-3xl font-bold text-primary">{{ i + 1 }}</span>
              </div>
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="grid lg:grid-cols-2 gap-16 items-center">
        <div class="order-2 lg:order-1" appAnimateOnScroll>
          <h2 id="about-heading" class="text-4xl lg:text-5xl font-black text-primary">Nossa Garantia: Qualidade e Confiança</h2>
          <p class="mt-6 text-lg text-slate-600 leading-relaxed">{{ vm.company.about }}</p>
          <ul class="mt-8 space-y-4 text-lg text-slate-700">
            <li class="flex items-start gap-3"><span class="text-accent font-bold text-2xl mt-1" aria-hidden="true">✓</span> <span><span class="font-bold">Orçamento Técnico Detalhado:</span> Sem surpresas, total transparência.</span></li>
            <li class="flex items-start gap-3"><span class="text-accent font-bold text-2xl mt-1" aria-hidden="true">✓</span> <span><span class="font-bold">Pontualidade e Limpeza:</span> Respeitamos seu tempo e seu espaço.</span></li>
            <li class="flex items-start gap-3"><span class="text-accent font-bold text-2xl mt-1" aria-hidden="true">✓</span> <span><span class="font-bold">Peças Originais:</span> Utilizamos apenas componentes que garantem a durabilidade.</span></li>
          </ul>
        ...
        <div class="order-1 lg:order-2" appAnimateOnScroll [style.transition-delay]="'150ms'">
          <img ngSrc="assets/images/arcond4.webp" alt="Técnico da PBC Instalação RJ realizando manutenção profissional" class="rounded-xl shadow-2xl w-full" width="1260" height="750" priority>
        </div>
      </div>
    </div>
  </section>
  ...
  <!-- TESTIMONIALS -->
  <section id="testimonials" aria-labelledby="testimonials-heading" class="py-24 lg:py-32 bg-light">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-16" appAnimateOnScroll>
        <h2 id="testimonials-heading" class="text-4xl lg:text-5xl font-black text-primary">Quem Confia, Recomenda</h2>
                ...
                <div class="flex items-center gap-1.5">
                  ...
                  @if (t.linkedin) {
                    <a [href]="t.linkedin" target="_blank" rel="noopener noreferrer" title="LinkedIn Profile"
                       class="text-slate-400 hover:text-primary transition-colors duration-200">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="max-w-4xl mx-auto" appAnimateOnScroll>
        <div class="text-center mb-16">
          <h2 id="faq-heading" class="text-4xl lg:text-5xl font-black text-primary">Perguntas Frequentes</h2>
        ...
        <div class="border border-slate-200 rounded-xl shadow-sm bg-light">
  ...
  <!-- FINAL CTA -->
  <section aria-labelledby="final-cta-heading" class="py-24 cta-bg text-white" appAnimateOnScroll>
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <a [href]="vm.whatsapp.normal" target="_blank" rel="noopener noreferrer" class="mt-8 inline-block bg-accent text-dark font-bold text-xl py-5 px-12 rounded-lg hover:scale-105 transition-transform duration-200 shadow-2xl">
  ...
  <!-- FOOTER -->
  <footer id="footer" role="contentinfo" class="bg-dark text-white pt-20 pb-8" appAnimateOnScroll>
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-12 text-center md:text-left">
        ...
        <div>
          ...
          <ul class="space-y-3">
            @for (s of vm.sections; track s.id) {
              <li><button (click)="scrollTo(s.id)" class="text-slate-400 hover:text-accent transition-colors">{{ s.label }}</button></li>
            }
          </ul>
        </div>
        <div>
      ...
      <div class="mt-16 border-t border-slate-800 pt-8 text-center text-slate-500">
        ...
        <p class="mt-4 text-sm">
          Desenvolvido por 
          <a href="https://www.linkedin.com/in/devferreirag" target="_blank" rel="noopener noreferrer" class="font-medium text-slate-400 hover:text-accent transition-colors">
            Gabriel Ferreira
          </a>
  ...
  <!-- Floating Mobile CTA -->
  <a [href]="vm.whatsapp.normal" target="_blank" rel="noopener noreferrer"
     class="fixed bottom-4 left-4 right-4 bg-accent text-dark font-black text-center py-4 rounded-full shadow-2xl z-30 lg:hidden text-lg flex items-center justify-center gap-2 hover:scale-105 transition-transform animate-fade-in-bottom">
    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.894 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.886-.001 2.269.655 4.398 1.919 6.121l-1.161 4.225 4.273-1.119z"/></svg>
    SOLICITAR ORÇAMENTO
  </a>
</main>
